...
#include <stdio.h>
#include <fcntl.h>
CardWindow::CardWindow(QWidget *parent) :QMainWindow(parent),ui(new Ui::CardWindow)
{
    int pd;
    char buf[32];
...
//set read time
void CardWindow::on_actionReadPortInterval_triggered()
{
    bool ok = false;
...
void CardWindow::closeEvent(QCloseEvent *e)
{
    exit(0);
 }

void CardWindow::moveEvent(QMoveEvent *)
{
    this->move(QPoint(0,0));
}

void CardWindow::resizeEvent(QResizeEvent *)
{
    this->showMaximized();
}
...
void CardWindow::on_nomal_clicked()
{
    ui->sendmsgBtn->setEnabled(true);
    ui->sendMsgLineEdit->setEnabled(true);
    ui->obocheckBox->setEnabled(true);
...
void CardWindow::on_send485_clicked()
{
    ui->sendmsgBtn->setEnabled(true);
    ui->sendMsgLineEdit->setEnabled(true);
    ui->obocheckBox->setEnabled(true);
...
void CardWindow::on_recv485_clicked()
{
